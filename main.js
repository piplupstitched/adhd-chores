...
var ADHDChoresSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Auto-generate on startup").addToggle((t) => t.setValue(this.plugin.data.settings.autoGenerateOnStartup).onChange((v) => {
      this.plugin.data.settings.autoGenerateOnStartup = v;
      void this.plugin.savePluginData();
    }));
    new import_obsidian.Setting(containerEl).setName("Rollover missed chores").addToggle((t) => t.setValue(this.plugin.data.settings.rolloverMissed).onChange((v) => {
      this.plugin.data.settings.rolloverMissed = v;
      void this.plugin.savePluginData();
    }));
    new import_obsidian.Setting(containerEl).setName("Quick focus task count").addText((t) => t.setValue(String(this.plugin.data.settings.focusTaskCount)).onChange((v) => {
      const n = parseInt(v || "3", 10);
      this.plugin.data.settings.focusTaskCount = Number.isFinite(n) ? Math.min(6, Math.max(1, n)) : 3;
      void this.plugin.savePluginData();
    }));
    new import_obsidian.Setting(containerEl).setName("Enable streaks and rewards").setDesc("Optional. Turn this off anytime.").addToggle((t) => t.setValue(this.plugin.data.settings.enableRewards).onChange((v) => {
      this.plugin.data.settings.enableRewards = v;
      void this.plugin.savePluginData();
      this.plugin.refreshView();
    }));
    new import_obsidian.Setting(containerEl).setName("Show gamification chips").addToggle((t) => t.setValue(this.plugin.data.settings.showGamification).onChange((v) => {
      this.plugin.data.settings.showGamification = v;
      void this.plugin.savePluginData();
      this.plugin.refreshView();
    }));
    new import_obsidian.Setting(containerEl).setName("Reminders").setHeading();
    new import_obsidian.Setting(containerEl).setName("Enable reminders").addToggle((t) => t.setValue(this.plugin.data.settings.remindersEnabled).onChange((v) => {
      this.plugin.data.settings.remindersEnabled = v;
      void this.plugin.savePluginData();
    }));
    new import_obsidian.Setting(containerEl).setName("Reminder times").setDesc("Comma-separated 24h times, e.g. 09:00,14:00,19:00").addText((t) => t.setValue(this.plugin.data.settings.reminderTimesCsv).onChange((v) => {
      this.plugin.data.settings.reminderTimesCsv = v.trim();
      void this.plugin.savePluginData();
    }));
    new import_obsidian.Setting(containerEl).setName("Logging").setHeading();
    new import_obsidian.Setting(containerEl).setName("Log events to note").addToggle((t) => t.setValue(this.plugin.data.settings.logToNote).onChange((v) => {
      this.plugin.data.settings.logToNote = v;
      void this.plugin.savePluginData();
    }));
    new import_obsidian.Setting(containerEl).setName("Log note path").addText((t) => t.setValue(this.plugin.data.settings.logNotePath).onChange((v) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Auto weekly reset").setDesc("Runs automatically at startup each week.").addToggle((t) => t.setValue(this.plugin.data.settings.autoWeeklyReset).onChange((v) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Auto-sync chore block when planning").setDesc("Writes/updates a chore block in each planned day note.").addToggle((t) => t.setValue(this.plugin.data.settings.syncDailyBlockOnPlan).onChange((v) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Daily note date format override").setDesc("Leave blank to use the core daily notes plugin date format.").addText((t) => t.setValue(this.plugin.data.settings.dailyNotesFormat).onChange((v) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Chore task tag").setDesc("Optional. Leave blank for no tag. Example: #task/chore").addText((t) => t.setValue(this.plugin.data.settings.choreTaskTag).onChange((v) => {
    ...
    for (const template of this.plugin.data.templates) {
      new import_obsidian.Setting(containerEl).setName(`${template.name} (${template.room})`).setDesc(this.plugin.describeTemplate(template)).addToggle((t) => t.setValue(template.active).onChange((v) => {
        template.active = v;
        void this.plugin.savePluginData();
        this.display();
