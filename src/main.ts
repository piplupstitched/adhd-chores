...
const VIEW_TYPE = 'adhd-chores-view';
...
interface BadgeCatalogItem {
	...
	label: string;
	description: string;
	targetType: 'level' | 'tasks' | 'streak' | 'quick';
	targetValue: number;
}

interface ChoreSettings {
	maxDailyMinutes: number;
	autoGenerateOnStartup: boolean;
	rolloverMissed: boolean;
	focusTaskCount: number;
	enableRewards: boolean;
	showGamification: boolean;
	remindersEnabled: boolean;
	reminderTimesCsv: string;
	logToNote: boolean;
	logNotePath: string;
	autoWeeklyReset: boolean;
	dailyNotesIntegrationEnabled: boolean;
	syncDailyBlockOnPlan: boolean;
	dailyNotesFolder: string;
	dailyNotesFormat: string;
	emitTasksPluginFormat: boolean;
	choreTaskTag: string;
}
...
interface ChoreMeta {
	...
	lastPlanDate: string;
	bodyDoubleEndsAt: string;
	bodyDoubleDurationMinutes: number;
}
...
function dayCode(day: number): string {
	const names = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
	return names[day] || String(day);
}

function parseReminderTimes(csv: string): string[] {
	return csv
		.split(',')
		.map(v => v.trim())
...
function standardTemplates(): ChoreTemplate[] {
	return [
...
function cloneDefaults(): ChoreData {
	return JSON.parse(JSON.stringify(DEFAULT_DATA)) as ChoreData;
}
...
class AddChoreModal extends Modal {
	private onSave: (template: ChoreTemplate) => Promise<void>;
	private nameVal = '';
	private roomVal = '';
	private frequencyVal: FrequencyType = 'weekly';
	private intervalVal = 1;
	private preferredDaysVal = '';
	private dayOfMonthVal = 1;
	private estMinutesVal = 10;
	private pointsVal = 10;

	constructor(app: App, onSave: (template: ChoreTemplate) => Promise<void>) {
		super(app);
		this.onSave = onSave;
	}
	...
	onOpen(): void {
		...
		new Setting(contentEl).setName('Name').addText(t => t.setPlaceholder('Wipe kitchen counters').onChange(v => (this.nameVal = v.trim())));
		...
		new Setting(contentEl).setName('Estimated minutes').addText(t => t.setValue(String(this.estMinutesVal)).onChange(v => {
			const n = parseInt(v || '10', 10);
			this.estMinutesVal = Number.isFinite(n) ? Math.max(1, n) : 10;
		}));
		new Setting(contentEl).setName('Points').addText(t => t.setValue(String(this.pointsVal)).onChange(v => {
			const n = parseInt(v || '10', 10);
...
class BodyDoubleModal extends Modal {
	private onPick: (minutes: number) => void;
	constructor(app: App, onPick: (minutes: number) => void) {
		super(app);
		this.onPick = onPick;
	}
	onOpen(): void {
...
class BadgesModal extends Modal {
	private plugin: ADHDChoresPlugin;
	constructor(app: App, plugin: ADHDChoresPlugin) {
		super(app);
		this.plugin = plugin;
	}
	...
	onOpen(): void {
		...
		for (const badge of catalog) {
			...
			if (unlocked) row.addClass('unlocked');
...
class ADHDChoresView extends ItemView {
	plugin: ADHDChoresPlugin;
	private selectedDate: string | null = null;
	constructor(leaf: WorkspaceLeaf, plugin: ADHDChoresPlugin) {
		super(leaf);
		this.plugin = plugin;
	}
	...
	render(): void {
		...
		if (!weekDates.includes(this.selectedDate)) this.selectedDate = today;
		...
		if (todayTasks.length === 0) {
			...
			for (const task of todayTasks) {
				...
				if (task.status === 'done') row.addClass('done');
...
class ADHDChoresSettingTab extends PluginSettingTab {
	plugin: ADHDChoresPlugin;
	constructor(app: App, plugin: ADHDChoresPlugin) {
		super(app, plugin);
		this.plugin = plugin;
	...
	display(): void {
		...
		containerEl.createEl('p', {
			text: 'Task query integration is optimized for the community tasks plugin. Other task plugins may not index checklist items the same way.',
			cls: 'adhd-empty',
		});
		...
		new Setting(containerEl).setName('Auto weekly reset').setDesc('Runs automatically at startup each week.').addToggle(t => t.setValue(this.plugin.data.settings.autoWeeklyReset).onChange(v => {
		...
		new Setting(containerEl)
			...
			.setDesc('Leave blank to use the core daily notes plugin date format.')
...
export default class ADHDChoresPlugin extends Plugin {
	data: ChoreData = cloneDefaults();
	private reminderTickId: number | null = null;
	private notifiedReminderKeys: Set<string> = new Set();
	private bodyDoubleTimeoutId: number | null = null;
	private bodyDoubleDisplayTickId: number | null = null;
	private bodyDoubleStatusEl: HTMLElement | null = null;
	...
	todayStr(): string { return fmtDate(new Date()); }

	getTasksForDate(date: string): ChoreTask[] {
	...
	getOverdueTasks(today: string): ChoreTask[] {
		return this.data.tasks.filter(t => t.status === 'pending' && t.dueDate < today).sort((a, b) => a.dueDate.localeCompare(b.dueDate));
	}

	getCompletedForDate(date: string): ChoreTask[] {
		return this.data.tasks.filter(t => t.status === 'done' && (t.completedAt || '').startsWith(date));
	}

	describeTemplate(template: ChoreTemplate): string {
		if (template.frequency === 'daily') return `daily | ${template.estMinutes}m`;
		if (template.frequency === 'every_n_days') return `every ${template.interval} days | ${template.estMinutes}m`;
	...
	openAddTemplateModal(): void {
		new AddChoreModal(this.app, async template => {
			...
			new Notice(`Added "${template.name}"`);
		}).open();
	}

	openBodyDoubleModal(): void {
		new BodyDoubleModal(this.app, minutes => { void this.startBodyDoubleSession(minutes); }).open();
	}

	isBodyDoubleActive(): boolean {
		return this.getBodyDoubleRemainingMs() > 0;
	}

	getBodyDoubleRemainingMs(): number {
	...
	getBodyDoubleRemainingLabel(): string {
	...
	getBadgeProgress(item: BadgeCatalogItem): { current: number; target: number } {
	...
	async startBodyDoubleSession(minutes: number): Promise<void> {
		...
		new Notice(`Body double started: ${minutes} minutes.`);
		await this.logEvent(`Started body double session (${minutes}m)`);
		this.refreshView();
	}

	async cancelBodyDoubleSession(): Promise<void> {
		if (!this.data.meta.bodyDoubleEndsAt) return;
		if (this.bodyDoubleTimeoutId !== null) {
			window.clearTimeout(this.bodyDoubleTimeoutId);
			this.bodyDoubleTimeoutId = null;
		}
		...
		new Notice('Body double cancelled.');
		this.refreshView();
	}

	async quickFocusMode(): Promise<void> {
		const today = this.todayStr();
		if (this.getTasksForDate(today).length === 0) await this.generateTodayPlan(false);
	...
	async completeTask(taskId: string): Promise<void> {
		...
		if (template) template.lastCompleted = this.todayStr();
		if (this.data.settings.enableRewards) this.applyRewards(task.points);
	...
	async snoozeTask(taskId: string, days = 1): Promise<void> {
		const task = this.data.tasks.find(t => t.id === taskId);
		if (!task || task.status === 'done') return;
	...
	async generateTodayPlan(showNotice = true): Promise<void> {
		const today = this.todayStr();
		await this.generatePlanForDate(today);
		if (showNotice) {
			const count = this.getTasksForDate(today).filter(t => t.status === 'pending').length;
			new Notice(`Planned ${count} chores for today.`);
		}
	}

	async runWeeklyReset(showNotice = true): Promise<void> {
		const today = this.todayStr();
		...
		// Reset only pending chores for this 7-day window, keep completed history.
		...
		this.refreshView();
		if (showNotice) new Notice(`Weekly reset complete: ${pendingAfter} chores planned (replaced ${pendingBefore}).`);
	}

	private async generatePlanForDate(date: string, writeLog = true): Promise<void> {
		if (this.data.settings.rolloverMissed && date === this.todayStr()) this.rolloverMissedTasks(date);

		for (const template of this.data.templates.filter(t => t.active)) {
			if (!this.shouldScheduleTemplateOnDate(template, date)) continue;
		...
		if (writeLog) await this.logEvent(`Auto-planned chores for ${date}`);
		this.refreshView();
	}

	private shouldScheduleTemplateOnDate(template: ChoreTemplate, date: string): boolean {
		...
		if (template.frequency === 'weekly') {
			...
			const doneRecently = this.data.tasks.some(t => {
				if (t.templateId !== template.id || t.status !== 'done') return false;
	...
	private rolloverMissedTasks(today: string): void {
		...
		for (const task of overdue) {
			if (todayTemplateIds.has(task.templateId)) continue;
			task.dueDate = today;
			todayTemplateIds.add(task.templateId);
		}
	}

	private applyDailyCap(date: string): void {
		...
		if (keep.size === pending.length) return;
		const nextDate = addDays(date, 1);
		for (const task of pending) {
			if (!keep.has(task.id)) task.dueDate = nextDate;
		}
	}

	private applyRewards(points: number): void {
		const today = this.todayStr();
	...
	private dedupeTasks(): void {
		const grouped = new Map<string, ChoreTask[]>();
	...
	private scheduleBodyDoubleCompletion(): void {
		if (this.bodyDoubleTimeoutId !== null) {
			window.clearTimeout(this.bodyDoubleTimeoutId);
			this.bodyDoubleTimeoutId = null;
		}
	...
	private async completeBodyDoubleSession(): Promise<void> {
		if (!this.data.meta.bodyDoubleEndsAt) return;
		...
		new Notice(`Body double complete. ${pending} pending chores left.`);
		await this.logEvent(`Completed body double session (${minutes}m)`);
		this.refreshView();
	}

	private restoreBodyDoubleSessionIfNeeded(): void {
		const remaining = this.getBodyDoubleRemainingMs();
		if (remaining <= 0) {
	...
	private startBodyDoubleDisplayTicker(): void {
	...
	private stopBodyDoubleDisplayTicker(): void {
		if (this.bodyDoubleDisplayTickId !== null) {
			window.clearInterval(this.bodyDoubleDisplayTickId);
			this.bodyDoubleDisplayTickId = null;
		}
	...
	private updateBodyDoubleStatus(): void {
		if (!this.bodyDoubleStatusEl) return;
		if (!this.isBodyDoubleActive()) {
	...
	private async checkMilestones(task: ChoreTask, levelBefore: number): Promise<void> {
	...
	private startReminderTicker(): void {
		if (this.reminderTickId !== null) window.clearInterval(this.reminderTickId);
		this.reminderTickId = window.setInterval(() => { void this.checkReminders(); }, 60 * 1000);
	}

	private async checkReminders(): Promise<void> {
		if (!this.data.settings.remindersEnabled) return;
		const now = new Date();
	...
	async logEvent(message: string): Promise<void> {
		if (!this.data.settings.logToNote) return;
		try {
	...
	async createOrOpenChoreDashboard(): Promise<void> {
		const tagRaw = this.data.settings.choreTaskTag?.trim() || '';
		const tag = !tagRaw ? '' : (tagRaw.startsWith('#') ? tagRaw : `#${tagRaw}`);
		const path = normalizePath('TaskNotes/Chore Dashboard.md');
	...
	private buildDailyNotePath(date: string): string {
		...
		const fileName = `${moment(date, 'YYYY-MM-DD').format(cfg.format)}.md`;
		if (!cfg.folder) return normalizePath(fileName);
		return normalizePath(`${cfg.folder}/${fileName}`);
	}
	...
	async syncWeekToDailyNotes(showNotice = true): Promise<void> {
		...
		if (showNotice) new Notice('Synced this week to daily notes.');
	}

	async loadPluginData(): Promise<void> {
		const loaded = (await this.loadData()) as Partial<ChoreData> | null;
